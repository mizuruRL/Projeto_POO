...
public abstract class RWFile {
     ...
     * @param fileNamePath Name of .txt file.
     * @param textToSave String content to save on .txt file.
     */
    public static void printToFile(String fileNamePath, String textToSave) {
        if(textToSave == null || textToSave.isEmpty())
            throw new IllegalArgumentException("Text to save cannot be null or empty.");
        Charset utf8 = StandardCharsets.UTF_8;
        try {
            Files.write(Paths.get(fileNamePath+ ".txt"), textToSave.getBytes(utf8));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
     ...
     * @param fileNamePath Name of .bin file.
     * @param obj Object to save.
     */
    public static void saveData(String fileNamePath, Object obj) {
        if(obj == null)
            throw new IllegalArgumentException("Data must exist.");
        try (FileOutputStream fout = new FileOutputStream(fileNamePath);
     ...
     * @param fileNamePath Name of .bin file to search.
     */
    public static Object loadData(String fileNamePath) {
        Object data = null;
        try (FileInputStream fin = new FileInputStream(fileNamePath);
